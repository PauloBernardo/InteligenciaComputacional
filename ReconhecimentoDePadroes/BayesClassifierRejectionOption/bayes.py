...
class BayesClassifier:
    ...
    def predict(self, x, threshold=0.1):
        ...
        for c in self.classes:
            ...
            total_likelihood += class_score
            class_scores.append(class_score)

        # print((np.max(class_scores)/total_likelihood), class_scores, self.class_prior, threshold, total_likelihood)
        predicted_class = self.classes[np.argmax(class_scores)] if (np.max(class_scores)/total_likelihood) > (0.5 + threshold) else -1
    ...
    def gaussian_density(self, x, class_mean, class_covariance):
        ...
        norm_const = 1 / ((2 * np.pi) ** (len(x) / 2) * np.linalg.det(class_covariance) ** 0.5)
